...
describe('PostDetailsController Test\n', function() {
    ...
    it("Should initialize controller with 1 post with title 'Test2'", inject(function($state, $stateParams, $rootScope, $controller, Post) {
        var $scope = $rootScope.$new();
        $stateParams.id = 2;
        $controller('PostDetailsController', {
            $scope: $scope,
        ...
        $httpBackend.flush(); // flush so that responses are actually sent
        expect($scope.singlePost.title).toBe('Test2');
    }));
});
