...
.controller('PostCreationController', ['$scope', '$state', 'Post', 'permalinkFilter', function($scope, $state, Post, permalinkFilter) {
    $scope.post = new Post();
    $scope.buttonText = "Create";
    $scope.savePost = function() {
        $scope.buttonText = "Saving. . .";
        // $scope.post.permalink = angular.lowercase($scope.post.title).replace(/[\s]/g, '-');
        $scope.post.permalink = permalinkFilter($scope.post.title);
